...
const MODES = {
    AGGRESSIVE: {
        MIN_PROB: 45,     // antes 55 → ahora entra mucho más
        STRONG_PROB: 52,  // antes 60+
        MIN_FREQ: 1,
        EMA_FAST: 8,
        EMA_SLOW: 34,
        MIN_SIGNAL_MS: 800,   // ~0.8s para confirmar
        STICK_MS: 2500,       // se queda pegada ~2.5s
        ALERT_GAP_MS: 7000,
        PROB_WINDOW: 4,
        VOL_WINDOW: 6,
        CONGESTION_TICKS: 2   // 2 ciclos seguidos mismo lado
    },
    BALANCED: {
        MIN_PROB: 50,
        STRONG_PROB: 56,
    ...
    const RESET_TIME = 30000;                       // reset de contadores (30s)
    ...
    // === PANEL MINI ===
    const existingPanel = document.getElementById("fx-mini-panel");
    if (existingPanel) {
        existingPanel.remove();
    }

    const panel = document.createElement("div");
    panel.id = "fx-mini-panel";
    panel.style = `
      position: fixed;
      top: 20px;
      right: 20px;
    ...
    `;
    panel.innerHTML = `
      <div id="fx_prob">Prob: ...%</div>
      <div id="fx_freq">Freq: ...</div>
      <div id="fx_trend">Trend: ...</div>
      <div id="fx_signal">Signal: ...</div>
      <div id="fx_state">State: ANALYZING</div>
    `;
    document.body.appendChild(panel);

    const probEl  = document.getElementById("fx_prob");
    const freqEl  = document.getElementById("fx_freq");
    const trendEl = document.getElementById("fx_trend");
    const sigEl   = document.getElementById("fx_signal");
    const stateEl = document.getElementById("fx_state");


           function getPrice() {
        try {
            // Recorremos spans y divs para buscar números que parezcan precio
            ...
            for (const el of elements) {
                ...
                // Evitar textos extremadamente largos (no son precios)
                if (text.length > 15) continue;

                // Limpiar todo lo que no sea dígito, punto, coma o signo
                const cleaned = text.replace(/[^\d.,-]/g, '');
                if (!cleaned) continue;

                // Debe tener al menos un punto o coma decimal
                ...
                // Guardamos este candidato (nos quedamos con el último)
                found = num;
            }
    ...
    function sendPanelSnapshotToN8n(probRaw, trend, signal, stateLabel, freq) {
       ...
       function calculate() {
        ...
        // === MOVIMIENTO TICK A TICK ===
        if (lastPrice !== null) {
            if (price > lastPrice) upTicks++;
            if (price < lastPrice) downTicks++;
            freq++;
        }
        ...
        // === PROBABILIDAD BRUTA POR RATIO DE TICKS ===
        const total = upTicks + downTicks;
        const probRaw = total > 0 ? (upTicks / total) * 100 : 0;
        lastProbRaw = probRaw;

        // Historial de probabilidad para suavizar
        probHistory.push(probRaw);
        if (probHistory.length > PROB_WINDOW) probHistory.shift();

        const smoothedProb = probHistory.reduce((a, b) => a + b, 0) / probHistory.length;

        // === VOLATILIDAD ADAPTATIVA ===
        volHistory.push(freq);
        if (volHistory.length > VOL_WINDOW) volHistory.shift();
        const avgVol = volHistory.reduce((a, b) => a + b, 0) / volHistory.length || freq;
        const dynamicMinFreq = Math.max(MIN_FREQ, Math.round(avgVol * 0.6));

        // === TENDENCIA MICRO + MACRO (EMA) ===
        const emaFast = calcEMA(priceHistory, EMA_FAST);
        const emaSlow = calcEMA(priceHistory, EMA_SLOW);

        let macroTrend = "none";
        if (emaFast && emaSlow) {
            if (emaFast > emaSlow) macroTrend = "up";
            if (emaFast < emaSlow) macroTrend = "down";
        }

        let microSlope = 0;
        if (emaFast !== null && lastEmaFast !== null) {
            microSlope = emaFast - lastEmaFast;
        }
        lastEmaFast = emaFast;

        // micro tendencia por pendiente
        const slopeUp = microSlope > 0;
        const slopeDown = microSlope < 0;

        // === CANDIDATO DE SEÑAL (sin confirmar) ===
        let candidateSide = "NO TRADE";

        const hasVolatility = freq >= dynamicMinFreq;

        if (hasVolatility && smoothedProb >= MIN_PROB) {
            // Tendencia combinada: macro + micro
            if (macroTrend === "up" && slopeUp) {
                candidateSide = "CALL";
            } else if (macroTrend === "down" && slopeDown) {
                candidateSide = "PUT";
            } else {
                candidateSide = "NO TRADE";
            }
        }

        // === MICRO CONGESTIÓN (lado se mantiene varios ciclos) ===
        sideHistory.push(candidateSide);
        if (sideHistory.length > CONGESTION_TICKS) sideHistory.shift();

        const allSameSide =
            sideHistory.length === CONGESTION_TICKS &&
            sideHistory.every(s => s === candidateSide && s !== "NO TRADE");

        const isStrongCandidate =
            allSameSide &&
            (candidateSide === "CALL" || candidateSide === "PUT") &&
            smoothedProb >= STRONG_PROB &&
            hasVolatility;

        const now = Date.now();

        // Actualizamos estado del candidato actual
        if (isStrongCandidate) {
            if (currentSignalSide !== candidateSide) {
                currentSignalSide = candidateSide;
                signalStartAt = now;
            }
        } else {
            // si ya pasó el tiempo de pegajosidad, limpiamos
            if (currentSignalSide !== "NO TRADE" &&
                now - signalStartAt > SIGNAL_STICK_TIME) {
                currentSignalSide = "NO TRADE";
                signalStartAt = 0;
            }
        }

        // === SEÑAL ESTABLE (confirmada en el tiempo) ===
        let stableSignal = "NO TRADE";
        let stateLabel = "ANALYZING";

        if (currentSignalSide === "CALL" || currentSignalSide === "PUT") {
            const elapsed = now - signalStartAt;

            if (elapsed >= MIN_SIGNAL_DURATION) {
                stableSignal = currentSignalSide;
                stateLabel = stableSignal === "CALL"
                    ? "ENTRY WINDOW UP"
                    : "ENTRY WINDOW DOWN";
            } else {
                stateLabel = "PRE-SIGNAL";
            }
        } else {
            stateLabel = "NO CLEAR SIGNAL";
        }

        // === ACTUALIZAR UI MINI PANEL ===
        if (probEl) {
            probEl.textContent = `Prob: ${smoothedProb.toFixed(1)}%`;
        }

        if (freqEl) {
            freqEl.textContent = `Freq: ${freq} (min ${dynamicMinFreq})`;
        }

        if (trendEl) {
            const label =
                macroTrend === "up" ? "UP" :
                macroTrend === "down" ? "DOWN" :
                "NONE";
            trendEl.textContent = `Trend: ${label}`;
        }

        const displaySignal =
            stableSignal !== "NO TRADE" ? stableSignal : currentSignalSide;

        if (sigEl) {
            sigEl.textContent = `Signal: ${displaySignal}`;
            sigEl.style.color =
                displaySignal === "CALL" ? "#4fff6a" :
                displaySignal === "PUT"  ? "#ff5b5b" :
                "#dddddd";
        }

        if (stateEl) {
            stateEl.textContent = `State: ${stateLabel}`;
        }

        // === ENVÍO A N8N SOLO PARA SEÑALES ESTABLES ===
    ...
    // Reset periódico de contadores de ticks / freq
    setInterval(() => {
        upTicks = 0;
        downTicks = 0;
        freq = 0;
